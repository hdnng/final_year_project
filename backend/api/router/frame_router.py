...
@router.get("/{session_id}", response_model=list[FrameResponse])
def get_frames(
    session_id: int,
    ...
    """List frames for a session with pagination."""
    try:
        return get_frames_by_session(db, session_id, skip, limit)
...
def get_analysis(
    ...
    return get_analysis_data(db, session_id)
